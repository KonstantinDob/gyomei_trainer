...
def save_configs(folder_path: str, config_path: str):
    ...
    """
    os.system(f"cp {config_path}/train.yaml {folder_path}/train.yaml")
    os.system(f"cp {config_path}/data/dataset.yaml {folder_path}/dataset.yaml")
    os.system(f"cp {config_path}/model/model.yaml {folder_path}/model.yaml")


def create_experiment_folder(project_path: Optional[str]) -> Optional[str]:
    ...
    if not os.path.exists(join(project_path, "configs")):
        RuntimeError(
            "There is no config directory!"
            "Check project_path argument in the Builder initialization"
        )
