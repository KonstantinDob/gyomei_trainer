...
from gyomei_trainer.modules.logs_formatter import (
    LogsFormatter, make_epoch_log
)
...
class TestFormatter:
    ...
    @pytest.mark.parametrize(
        ...
        [(-5.0, '', AverageValueMeter(),
          {'': AverageValueMeter()}, -20),
         (0.0, '123', AverageValueMeter(), {}, 0),
         (1e3, '-'*int(1e2), AverageValueMeter(),
          {'0'*int(1e2): AverageValueMeter()}, 1e6)]
    )
    def test_logs_formatter(
            self, timer: float, loss_name: str,
            loss_value_valid: AverageValueMeter,
            metrics_name: Dict[str, AverageValueMeter], epoch: int
    ):
        ...
        state = State()
        state.update(logger=logger, timer=timer, loss_name=loss_name,
                     loss_value_valid=loss_value_valid,
                     metrics_name=metrics_name, epoch=epoch)
        formatter = LogsFormatter(state)
        formatter.epoch_complete(state)

    @pytest.mark.parametrize(
        'seconds, metric_data, epoch',
        [(10.0, {'loss': AverageValueMeter(),
                 'accuracy': AverageValueMeter()}, 5),
         (-5.2, {'': AverageValueMeter(),
                 '--': AverageValueMeter()}, -5),
         (66.6, {}, 0)]
    )
    def test_make_epoch_log(self, seconds: float,
